...
class _EditStaffMemberScreenState extends State<EditStaffMemberScreen> {
 ...
 Future<void> _fetchStaffData() async {
  ...
  try {
    ...
    ?.map((venue) => venue["id"].toString()) // Extract only the ID as a string
    .toList() ?? [];

_selectedPermissions = (data["permissions"] as List<dynamic>?)
    ?.map((permission) => permission["id"].toString()) // Extract only the ID as a string
    .toList() ?? [];
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: AppConstants.firstNameField(controller: _firstNameController),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: AppConstants.lastNameField(controller: _lastNameController),
                      ),
                    ],
                  ),
