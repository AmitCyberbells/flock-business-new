...
class _OpenHoursScreenState extends State<OpenHoursScreen> {
  ...
  final List<Map<String, dynamic>> _baseDays = [
    {"day": "Mon", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Tue", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Wed", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Thu", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Fri", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Sat", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
    {"day": "Sun", "isOpen": false, "openTime": "", "closeTime": "", "updated": false},
  ];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success' && data['data'] != null) {
          ...
          final loadedVenues = venueData
              .map<Map<String, dynamic>>((v) => {'id': v['id'], 'name': v['name'] ?? 'Unnamed Venue'})
              .toList();
          setState(() {
  ...
  Future<void> _fetchOpeningHours(int venueId) async {
    ...
    final url = Uri.parse('https://api.getflock.io/api/vendor/venues/$venueId/opening-hours');
    try {
      final response = await http.get(
        url,
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success' && data['data'] != null) {
          ...
          final updatedDays = _baseDays.map((e) => Map<String, dynamic>.from(e)).toList();
          ...
          for (final item in hoursData) {
            ...
            if (idx != -1) {
              updatedDays[idx] = {
                "day": localDay,
                "isOpen": isActive,
                "openTime": _convertTo12HrFormat(openTime.isEmpty ? "12:00 AM" : openTime),
                "closeTime": _convertTo12HrFormat(closeTime.isEmpty ? "12:00 AM" : closeTime),
                "updated": false,
              };
            }
          }
        ...
        setState(() {
          _errorMessage = 'Error ${response.statusCode}: Unable to fetch hours.';
        });
      }
    } catch (e) {
      setState(() {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (data['status'] == 'success') {
          ...
          setState(() => _errorMessage = data['message'] ?? 'Failed to update hours.');
        }
      } else {
        setState(() => _errorMessage = 'Error ${response.statusCode}: ${response.body}');
      }
    } catch (e) {
      setState(() {
        _isLoading = false;
  ...
  TimeOfDay _parseFlexible(String s) {
    ...
    try {
      ...
      return TimeOfDay(hour: int.parse(parts[0]) % 24, minute: int.parse(parts[1]) % 60);
    } catch (_) {
  ...
  List<DaySchedule> _buildWeekFromDays(List<Map<String, dynamic>> days) {
    ...
    return order.map((d) {
      final row = days.firstWhere(
        (e) => e['day'] == d,
        orElse: () => {'isOpen': false, 'openTime': '12:00 AM', 'closeTime': '12:00 AM'},
      );
      ...
      body: Stack(
        children: [
                    ...
                    child: Row(
                      children: [
                        InkWell(
                          ...
                          child: Image.asset('assets/back_updated.png', height: 40, width: 34),
                        ),
                        Expanded(
                          child: Center(
                            child: Text(
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(_selectedVenue?['name'] ?? 'Select a venue',
          ...
          if (_isLoading)
            Stack(
              children: [
                Container(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.14)),
                Container(
          ...
          children: [
              ...
              child: GestureDetector(
                onTap: () {
                  setState(() {
                    ...
                    if (!_days[index]["isOpen"]) _days[index]["updated"] = false;
                    _recomputeOpenNow();
                  });
                },
                child: Container(
            ...
            Expanded(
              child: Row(
                ...
                children: [
                  Text(dayInfo["openTime"], style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 14)),
                  Text(" - ", style: Theme.of(context).textTheme.bodyMedium),
                  Text(dayInfo["closeTime"], style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 14)),
                ],
              ),
            ),
            IconButton(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text("Apply to all days", style: Theme.of(context).textTheme.bodyMedium),
                    ],
                  ),
                ],
              ),
